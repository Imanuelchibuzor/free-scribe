...
export default function Homepage() {
  ...
  const handleTranscribe = () => {
    if (!transcribedText) {
      ...
      navigate("/output", {
        state: { language: userLanguage, message: transcribedText },
      });
    }
  };
    ...
    <main className="flex-1 p-4 flex flex-col gap-3 text-center sm:gap-4 justify-center pb-20">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {duration !== 0 && (
            <p className="text-sm text-blue-400 font-medium">{duration}s</p>
          )}
          <i
            className={
              "fa-solid duration-200 fa-microphone " +
              (isListening === true ? "text-rose-600" : "")
            }
          ></i>
        </div>
      </button>
      ...
      {transcribedText ? (
        <p className="text-base font-medium text-blue-400 italic">
          Speech Recorded
        </p>
      ) : null}
